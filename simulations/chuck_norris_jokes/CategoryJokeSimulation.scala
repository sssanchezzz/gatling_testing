...
class CategoryJokeSimulation extends Simulation {
  ...
  private val scn = scenario("CategoryJokeSimulation")
    .exec(
      http("request_0")
        .get("/jokes/random?category=dev")
    )

setUp(scn.inject(rampUsers(500).during(15.seconds))).protocols(httpProtocol)
}
